// Tektronix - TBS1062 - get settings Version 02
// Gustavo Murta - 2025_11_17
...
// C:\Users\jgust\go\programas\Tektronix\tek_get_settings\Version02
// Version 02 - 2025_11_17 -  fix read buffer - read until Line Feed (0x0A) - remove extra bytes
...
const (
	vid = gousb.ID(0x0699) // TBS1062 scope
	pid = gousb.ID(0x03B0)
)
...
func readScope(epInR *gousb.InEndpoint, dataLenght int) []byte {
	...
	for i := 0; i < dataLenght; i++ {
		buffer := readBuffer[i]
		bufferOutput = append(bufferOutput, buffer) // append byte to output buffer

		if buffer == 0x0A { // Line Feed (end of data)
			readBytes = len(bufferOutput) - 1 // adjust read bytes count
			break
		}
	}
	readBuffer = bufferOutput[:readBytes] // adjust read buffer size

	log.Printf("Read data HEXA (%d bytes): % X", readBytes, readBuffer)          // print read buffer Hexadecimal
	log.Printf("Read data ASCII: (%d bytes): %s", readBytes, string(readBuffer)) // print read buffer ASCII
...
func main() {
	...
	if err != nil {               // Se erro for diferente de nada
		log.Println("ERROR: fail to detach")
		return
	}
	...
	////////////////////////////////////////////////////////////////////////////
	// Reading data from SCOPE - URB_BULK in
	settingsBuffer = append(settingsBuffer, readScope(epIn05, 1036)...) // read 1036 bytes (1024 + 12 header)
